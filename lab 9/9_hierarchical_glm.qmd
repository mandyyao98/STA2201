...
stan_data=list(y=observe.i,
               ...
               N=length(observe.i),
               x=aff.i-mean(aff.i))
mod1=stan(data=stan_data,file=here("lab9_1.stan"))
```
## Model 2
```{r}
#| output: false
mod2=stan(data=stan_data,file=here("lab9_2.stan"))
mod2
```
...
```{r}
#| output: false
mod3=stan(data=stan_data,file=here("lab9_3.stan"))
mod3
```
...
## Answer
The first plot shows a histogram of log theta estimates for each model. The second plot shows the log estimates for each region with error bars, with colour indicating each model for the estimates and the size of the points indicating the number of observed deaths in each region. 
